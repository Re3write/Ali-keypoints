...
def main(args):
    ...
    checkpoint_file = os.path.join('model', 'checkpoint', 'epoch19checkpoint_dr_newlr.pth.tar')
    ...
    if not isdir(result_path):
        mkdir_p(result_path)
    result_file = os.path.join(result_path, 'result50_19_dr45.csv')
    with open(result_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(full_result)

    Evaluator = FaiKeypoint2018Evaluator(userAnswerFile=os.path.join(result_path, 'result50_19_dr45.csv'),
