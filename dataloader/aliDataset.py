...
class MscocoMulti(data.Dataset):
    ...
    def __getitem__(self, index):
        a = self.datalist[index]
        img_path = a["imgpath"]
        # print(img_path)
        if self.is_train:
            points = np.array(a['joints']).reshape(self.num_class, 3).astype(np.float32)
            # print(points)
        gt_bbox = a['bbox']
        # print(a["operation"])
        # print(gt_bbox)
        ...
        if self.is_train:
            ...
            for i in range(self.num_class):
                if pts[i, 2] > -0.5: # ali visible: -1-no label, 0-label + invisible, 1-label + visible
        ...
        if self.is_train:
            meta = {'index': index, 'GT_bbox': np.array([gt_bbox[0], gt_bbox[1], gt_bbox[2], gt_bbox[3]]),
