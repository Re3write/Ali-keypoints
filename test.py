...
from preprocessing.dataset import preprocessing,preprocessing_crop
from dataloader.tianchi_keypoint_dataset import get_tianchi_test_dataset
        ...
        while img_start < det_range[1]:
            ...
            for test_id in range(0, len(test_data), batch_size):
                ...
                for test_image_id in range(start_id, end_id):
                    ...
                    border = 10
                    dr = np.zeros((config.KEYPOINTS_NUM, config.OUTPUT_SHAPE[0] + 2 * border,
                                   config.OUTPUT_SHAPE[1] + 2 * border))
            ...
            cls_skeleton = np.concatenate(
                [cls_skeleton.reshape(-1, config.KEYPOINTS_NUM * 3),
                 (cls_scores * cls_partsco.mean(axis=1))[:, np.newaxis]],
                axis=1)
            for i in range(len(cls_skeleton)):
                # result = dict(image_id=im_info['imgpath'], category_id=im_info["category"], score=float(round(cls_skeleton[i][-1], 4)),
                #               keypoints=cls_skeleton[i][:-1].round(3).tolist())
                result = []
                result.append(im_info['imgid'])
                result.append(im_info["category"])
                # print(len(cls_skeleton[i][:-1].round(3).tolist()))
                j=0
                while j<len(cls_skeleton[i][:-1]):
                    result.append(str(int(cls_skeleton[i][j]))+'_'+str(int(cls_skeleton[i][j+1]))+'_1')
                    j+=3
                dump_results.append(result)
    return all_res, dump_results


def test(test_model,result_file):
    # loading model
    ...
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    result_path = osp.join(output_dir, result_file)

    with open(result_path, 'w', newline='') as f:
        #json.dump(dump_results, f)
        writer = csv.writer(f)
        writer.writerows(dump_results)
    K.clear_session()
...
if __name__ == '__main__':
    def parse_args():
        ...
        parser.add_argument('--model', '-m', default="./logs/resnet50_cpn20181017T1431/cpn_resnet50_dilated5ADAM_epoch036-loss25.714-val_loss30.199241.h5", type=str, dest='test_model')
    ...
    args = parse_args()
    if args.test_model:
        result_file = "results1017_36.csv"
        test(args.test_model,result_file=result_file)
        # from IPython import embed; embed()
        Evaluator = FaiKeypoint2018Evaluator(userAnswerFile=osp.join("logs",result_file),
